...
st.set_page_config(
    page_title="Loan Affordability Instrument",
    page_icon="💳",
    layout="centered",
...
"ru": {
    ...
    # welcome
    "hero_title":   "💳 Loan Affordability Instrument",
    ...
    "ml_proba_title": "Уверенность модели (Decision Tree) — вероятность уровня нагрузки по месяцам",
...
**Макроэкономический уровень нагрузки** определяется обученной моделью **Decision Tree** (дерево решений, max_depth=4, class_weight=balanced). Модель обучена на исторических месячных данных Узбекистана (2010–2025, 192 наблюдения). Признаки: инфляция, ставка рефинансирования, CPI MoM, номинальная зарплата, реальный рост зарплат, реальная ставка, индикатор долговой нагрузки. Целевая переменная: Low / Medium / High.

По результатам анализа (диссертация): точность Decision Tree на тестовой выборке — 79.5%, F1-macro — 0.760. Модель выбрана за прозрачность и интерпретируемость правил (Explainable AI).
...
Инструмент не принимает решений за банк и не является финансовым советником.""",
    ...
    "mc_default_sub":   "Monte Carlo · Decision Tree",
    ...
    "mf_sub":       "Decision Tree · на основе исторических данных Узбекистана 2010–2025",
    "mf_infl":      "Инфляция",
...
"en": {
    ...
    "ml_proba_title": "Model confidence (Decision Tree) — predicted pressure probability per month",
...
**Macro-level pressure** is classified by a trained **Decision Tree** model (max_depth=4, class_weight=balanced), fit on monthly Uzbekistan macro-financial data (2010–2025, 192 observations). Features: inflation, policy rate, CPI MoM, nominal wage, real wage growth, real policy rate, debt burden indicator. Target: Low / Medium / High.

Model performance (dissertation analysis): Decision Tree test accuracy 79.5%, F1-macro 0.760. Selected for its transparency and interpretable decision rules (Explainable AI).
...
The tool does not make bank decisions or provide financial advice.""",
    ...
    "mf_sub":       "Decision Tree · based on Uzbekistan historical data 2010–2025",
    "mf_infl":      "Inflation",
    "mf_rate":      "Policy rate",
...
"uz": {
    ...
    "ml_proba_title": "Model ishonchi (Decision Tree) — oylar bo'yicha bosim ehtimolligi",
...
**Makro bosim darajasi** o'qitilgan **Decision Tree** modeli (qaror daraxti, max_depth=4, class_weight=balanced) tomonidan aniqlanadi. Model O'zbekistonning oylik makroiqtisodiy ma'lumotlari asosida o'qitilgan (2010–2025, 192 kuzatuv). Belgilar: inflyatsiya, qayta moliyalashtirish stavkasi, CPI MoM, nominal ish haqi, real ish haqi o'sishi, real stavka, qarz ko'rsatkichi. Maqsad: Low / Medium / High.

Model ko'rsatkichlari: Decision Tree test aniqligi 79.5%, F1-macro 0.760. Model shaffoflik va tushunarli qoidalari uchun tanlangan (Explainable AI).
...
Vosita bank qarorlarini qabul qilmaydi.""",
    ...
    "mf_sub":       "Decision Tree · O'zbekiston tarixiy ma'lumotlari 2010–2025 asosida",
    "mf_infl":      "Inflyatsiya",
...
# ══════════════════════════════════════════════════════════════
# ML MODEL — train DecisionTree on historical macro data
# ══════════════════════════════════════════════════════════════
@st.cache_resource(show_spinner=False)
def train_model(_df: pd.DataFrame):
    """Train DecisionTreeClassifier on the historical macro dataset.
...
def ml_pressure(row, clf, le) -> str:
    """Predict macro pressure level using the trained Decision Tree model."""
    feat = np.array([[float(row.get(c, 0) or 0) for c in FEATURE_COLUMNS]])
    pred = clf.predict(feat)[0]
    return le.inverse_transform([pred])[0]

def ml_pressure_proba(row, clf, le) -> dict[str, float]:
    """Return {class: probability} dict from the Decision Tree model."""
    feat = np.array([[float(row.get(c, 0) or 0) for c in FEATURE_COLUMNS]])
...
def simulate(u: Inp, fc: pd.DataFrame) -> dict:
    ...
    for _, r in fc.iterrows():
        ...
        # by the Decision Tree (loan_risk_model.pkl) — see `res` below.
    ...
    # ── FINAL VERDICT — single decision-maker is the Decision Tree ──
